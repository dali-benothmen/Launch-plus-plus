...
For server behavior, the recommended feasibility prototype is QuickJS compiled to WebAssembly, inside a supervised worker with fresh invocation contexts, memory limits, execution deadlines and bounded broker calls. Expose only the SDK bridge. Retain the ability to change the runtime behind that protocol.

This is a proposed isolation architecture, not a security guarantee. Validate cancellation, async bridging, memory accounting and host bindings before accepting untrusted executable plugins. A worker alone is not a security boundary; Node’s vm module explicitly is not one either. The QuickJS wrapper documents runtime limits and async integration, which makes it a concrete prototype candidate. [Node documentation](https://nodejs.org/api/vm.html), [QuickJS wrapper documentation](https://github.com/justjake/quickjs-emscripten)
