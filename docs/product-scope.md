...
A solo founder, developer, freelancer, or small team can begin locally with almost no operational work. When the team grows, the same workspace can move to a VPS. Plugins add workflow-specific capabilities; themes change appearance; the core remains understandable.
...
1. **Minimal by default.** A new workspace has only broadly useful concepts.
...
4. **User-owned.** Workspaces can be backed up, exported, restored, and moved without a proprietary hosted dependency.
...
| Workspaces | Create, rename, switch, export | A user may belong to multiple workspaces |
| Membership | Invite, remove, change role | Owner, admin, member; guest is deferred |
| Projects | Create, archive, restore, delete | Workspace-owned; project access policy is explicit |
...
| Assignees | Assign workspace members | Multiple assignees supported |
| Labels | Workspace/project labels | Multiple labels per task |
...
| Permissions | Workspace and project policy | Centralized policy service, deny by default |
| Extensions | Install, enable, disable plugins/themes | Data is retained on disable |
| Portability | Backup, workspace export/import | Secrets excluded from portable exports |
...
- Statuses are project-scoped; a workspace template system is deferred.
...
Controls the server installation, executable plugin packages, storage location, backups, updates, and instance-wide policy. On a local solo installation, this is the same person as the workspace owner.

### Workspace owner

Controls workspace deletion, ownership transfer, membership, roles, installed package enablement, exports, and workspace defaults.

### Workspace admin

Manages projects, members within policy, workspace settings, and approved plugins. Cannot silently assume installation-operator powers on a hosted or shared server.
...
3. Complete the light first-owner path: create the owner account, name the workspace, and name the first project.
...
1. Open the last active workspace and project.
...
2. A workspace administrator reviews contributions, permissions, provenance, and compatibility.
3. The administrator enables it for the workspace and selected projects.
...
1. Export or back up the local workspace.
2. Install the same Launch++ release on the VPS.
3. Import the workspace and required package archives.
...
- **Global icon rail:** Home, product areas, approved extension pages, settings, and user menu.
- **Main content:** the Home project explorer, selected project view, Members, Settings, or a plugin page.
...
- URLs identify workspace, project, view, and selected task where appropriate.
...
| --- | --- |
| First local use | From supported install command to usable workspace in under five minutes |
...
- A new user creates a workspace, project, and first task without reading documentation.
...
- A workspace can move from a laptop to a fresh VPS without losing core or plugin data.
