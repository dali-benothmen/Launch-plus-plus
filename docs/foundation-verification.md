...
| --- | --- | --- |
| Production-like lifecycle with SQLite | `application-server.test.ts` constructs the production configuration, migrates a file-backed database, reaches readiness, closes all application and Better Auth connections, and safely reopens the same database. The core alpha also includes the first local/container distribution and guarded backup/restore commands. | The complete operator journey, TLS example, and release-level backup/restore qualification remain later work. |
