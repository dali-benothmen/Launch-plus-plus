...
- [x] **P1-06 — Build the authenticated application shell.** Implement the global icon rail, project-tree sidebar, responsive collapse/drawer behavior, workspace switching, route boundaries, and required unavailable/forbidden/error states.
- [x] **P1-06a — Build My Work.** Add assigned tasks, due-soon tasks, recently updated relevant tasks, recent/favorite projects, and a clear empty-state path to create or open a project. Keep analytics and configurable dashboard widgets outside core.
